...
server <- function(input, output, session) {
  ...
  # 读取使用说明文本
  usage_text <- paste(readLines("~/R/RNA/data/NASH/网页客户端/usage.txt", encoding = "UTF-8"), collapse = "<br/>")
  ...
  output$usage_ui <- renderUI({
    if (input$update == 0) {
      div(
        id = "usage_section",
        HTML(paste0("<div style='font-size:16px; line-height:1.6;'>", usage_text, "</div>"))
      )
    }
  })

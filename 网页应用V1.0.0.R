...
analyze_gene_expression_GSE59045 <- function(GeneX, vsd, colData) {
  # Step 1: 检查基因是否存在于表达矩阵
  if (!(GeneX %in% rownames(vsd))) {
    stop("目标基因不存在于表达矩阵中，请检查数据是否包含该基因。")
  }
  
  # Step 2: 获取目标基因的表达值，并转换成数据框
  gene_expression <- as.numeric(vsd[GeneX, ])
  
  # 确保 colData 也是数据框，并且行数匹配
  if (length(gene_expression) != nrow(colData)) {
    stop("表达数据和分组数据的样本数不匹配！")
  }
  
  gene_df <- data.frame(
    Sample = rownames(colData),  # 添加样本名称以避免顺序问题
    Expression = gene_expression,
    Condition = colData$condition,
    stringsAsFactors = FALSE
  )
  
  # Step 3: 仅保留 "Fibrosis_0_1" 和 "Fibrosis_3_4" 组
...
analyze_gene_expression_GSE49541 <- function(GeneX, vsd, colData) {
  # Step 1: 检查基因是否存在于表达矩阵
  if (!(GeneX %in% rownames(vsd))) {
    stop("目标基因不存在于表达矩阵中，请检查数据是否包含该基因。")
  }
...
analyze_gene_expression_GSE246221_Mouse <- function(GeneX, vsd, colData) {
  ...
  if (!(gene_id %in% rownames(vsd_GSE246221_Mouse))) {
    stop("目标基因不存在于表达矩阵中，请检查数据是否包含该基因。")
  }
  
  # Step 3: 获取目标基因的表达值
  gene_expression <- assay(vsd_GSE246221_Mouse)[gene_id, ]
  gene_df <- data.frame(
    Expression = gene_expression,
    Condition = colData_GSE246221_Mouse$condition
  )
...
# PASSWORD <- "1234"
users_db <- data.frame(
  username = c("admin", "user1", "user2", "testuser"),
  password = c("1234", "abcd", "5678", "testpass"),  
  stringsAsFactors = FALSE
)
    ...
    div(id = "sidebar_panel",
        ...
        conditionalPanel(
          ...
          radioButtons("analysis_type", "选择分析类型", 
                       choices = c("Summary", "Classification", "NAFLD Score", "Fibrosis Stages", "scRNAseq", "多基因分析"))
        ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$login, {
    ...
    # 验证用户名和密码
    valid_user <- users_db$username == input$user
    valid_pass <- users_db$password == input$password
    
    if (any(valid_user & valid_pass)) {
  ...
  analyze_Mouse_STAM_singlegene <- function(Gene) {
    ...
    datasets <- list(
      ...
      GSE246221 = list(data = vsd_GSE246221_Mouse, colData = colData_GSE246221_Mouse, func = analyze_gene_expression_GSE246221_Mouse)
    )
